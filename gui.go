...
const ScuffedInstall = "#scuffed-install"
...
func RawInfoModal(id, title, description string, isOpenAsar bool) g.Widget {
	isDynamic := strings.HasPrefix(id, ModalPrefix) && !strings.Contains(description, "\n")
	return g.Style().
			...
			g.PopupModal(id).
				...
				Layout(
					g.Align(g.AlignCenter).To(
						...
						&CondWidget{id == ScuffedInstall, func() g.Widget {
							return g.Column(
								g.Dummy(0, 10),
...
func ShowModal(title, desc string) {
	...
	modalMessage = desc
	modalId++
	g.OpenPopup(ModalPrefix + strconv.Itoa(modalId))
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		InfoModal(ScuffedInstall, "Hold On!", "You have a broken Discord Install.\n"+
		...
		InfoModal(ModalPrefix+strconv.Itoa(modalId), modalTitle, modalMessage),
	}
